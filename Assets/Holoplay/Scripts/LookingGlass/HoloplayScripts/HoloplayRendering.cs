...
namespace LookingGlass
{
    internal static class HoloplayRendering
    {
        ...
        //private static bool splitView = false;
        ...
        internal static void RenderQuilt(Holoplay holoplay, Action<int> onViewRender)
        {
            ...
            if (reader.user_num == 0)
            {
                camera1 = GameObject.Find("camera1");
                cam1 = camera1.GetComponent<Camera>();
            }
            else if (reader.user_num == 1)
            {
                camera1 = GameObject.Find("camera1");
                ...
                center = (int)((48 * reader.center_angle) / 70);
            }
            else if (reader.user_num >= 2)
            {
                camera1 = GameObject.Find("camera1");
                camera2 = GameObject.Find("camera2");
                cam1 = camera1.GetComponent<Camera>();
            ...
            ////determine_camera1
            Matrix4x4 centerViewMatrix = cam1.worldToCameraMatrix;
            Matrix4x4 centerProjMatrix = cam1.projectionMatrix;
            Matrix4x4 centerViewMatrix_sub = cam1.worldToCameraMatrix;//
            Matrix4x4 centerProjMatrix_sub = cam1.projectionMatrix;//
            if (reader.user_num == 0)
            {
                centerViewMatrix = singleViewCamera.worldToCameraMatrix;
                centerProjMatrix = singleViewCamera.projectionMatrix;
            }
            else if (reader.user_num == 1 && reader.format_tracking == 0 && reader.format_split == 0)
            {
                centerViewMatrix = singleViewCamera.worldToCameraMatrix;
                centerProjMatrix = singleViewCamera.projectionMatrix;
            }
            else if (reader.user_num == 1 && reader.format_tracking == 1 && reader.format_split == 0)
            {
                centerViewMatrix = cam1.worldToCameraMatrix;
                centerProjMatrix = cam1.projectionMatrix;
            }
            else if (reader.user_num == 1 && reader.format_tracking == 1 && reader.format_split == 1)
            {
                centerViewMatrix = cam1.worldToCameraMatrix;
                centerProjMatrix = cam1.projectionMatrix;
                centerViewMatrix_sub = cam2.worldToCameraMatrix;
                centerProjMatrix_sub = cam2.projectionMatrix;
            }
            else if (reader.user_num >= 2 && reader.format_split == 0)
            {
                centerViewMatrix = cam1.worldToCameraMatrix;
                centerProjMatrix = cam1.projectionMatrix;
            }
            else if (reader.user_num >= 2 && reader.format_split == 1)
            {
                centerViewMatrix = cam1.worldToCameraMatrix;
                centerProjMatrix = cam1.projectionMatrix;
                centerViewMatrix_sub = cam2.worldToCameraMatrix;
            ...
            for (int i = 0; i < quiltSettings.numViews; i++)
            {
                ...
                if (reader.user_num == 0)
                {
                    ...
                    projMatrix = centerProjMatrix;
                }
                else if (reader.user_num == 1 && reader.format_split == 0)
                {
                    viewMatrix = centerViewMatrix;
                    projMatrix = centerProjMatrix;
                }
                ...
                else if ((reader.user_num >= 2 && reader.format_split == 1) || (reader.user_num == 1 && reader.format_tracking == 1 && reader.format_split == 1))
                {
                    if (i >= 0 && i < 23)  //左側 (i>=0&&i<center-1) || (i < quiltSettings.numViews/2)         ////変更!!!!!!!!!!!!!!!!
                    {
                        if ((reader.user1_index >= 0 && reader.user2_index >= 0) && (openface.faces[reader.user1_index].GetValue("pose_Tx") >= openface.faces[reader.user2_index].GetValue("pose_Tx")))
                        {
                            viewMatrix = centerViewMatrix; // cam1.worldToCameraMatrix;
                            projMatrix = centerProjMatrix; // cam1.projectionMatrix;
                        }
                        else if ((reader.user1_index >= 0 && reader.user2_index >= 0) && (openface.faces[reader.user1_index].GetValue("pose_Tx") < openface.faces[reader.user2_index].GetValue("pose_Tx")))
                        {
                            viewMatrix = centerViewMatrix_sub;
                            projMatrix = centerProjMatrix_sub;
                        }
                    }
                    else if (i >= 23 && i <= 24) //(i>=center-1 && i<=center-1)
                    {
            ...
            singleViewCamera.worldToCameraMatrix = centerViewMatrix;  //変更
            singleViewCamera.projectionMatrix = centerProjMatrix;  //変更
