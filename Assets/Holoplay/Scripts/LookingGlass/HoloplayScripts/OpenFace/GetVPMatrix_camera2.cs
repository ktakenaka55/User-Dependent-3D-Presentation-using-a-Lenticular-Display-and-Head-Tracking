...
public class GetVPMatrix_camera2 : MonoBehaviour
{
    Camera cam;
    Vector3 initAng;
    Vector3 originPos;
    ...
    void Update()
    {
        if (reader.user2_index < 0) return;
        ...
        {
            originPos = get_cameraOriginPos(reader.user2_obj);

            float xAng = reader.angle_x[reader.user2_index] * rotation_speed;
            float yAng = reader.angle_y[reader.user2_index] * rotation_speed;
            ...
            //UnityEngine.Debug.Log($"user2_camera_Pos:{position}");
        }
    }

    private Vector3 get_cameraOriginPos(string user2_obj)
    {
        Vector3 position = new Vector3(0f, 0f, 0f);
        if (SceneManager.GetActiveScene().name == "Sample")
        {
            obj = GameObject.Find(user2_obj);
